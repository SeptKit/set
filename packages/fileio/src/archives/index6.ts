...
export function createXmlImporter(): XmlImporter {
	 ...
	 */
	async function createTableIfNeeded(state: ParserState, tableName: string, attributes: Record<string, any>) {
	 ...
	 */
	async function updateDatabaseSchema(state: ParserState, tableName: string, schema: Record<string, string>) {
		if (!state.db) {
	...
	function sendProgress(state: ParserState, force = false) {
		...
		if (handlers.onProgress) {
			handlers.onProgress({
				...
				percentComplete: state.totalBytes > 0
					? Math.round((state.bytesProcessed / state.totalBytes) * 100)
					: 0
			});
		}
	}
	...
	async function parseXmlContent(state: ParserState, content: string): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			parser.onclosetag = function (tagName) {
				...
				state.currentElement = state.currentPath.length > 0 ? state.currentPath[state.currentPath.length - 1] : null;
				state.currentAttributes = {};
				state.currentText = '';
